...
#if defined(NDEBUG)
# error "LyraCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfb554f778c8580f04aab3eabbac1d8fe921e2ed6e491865f1800c4941a83b37a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LyraCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LyraCoin Signed Message:\n";
...
{
    // LyraCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 780000); // LyraCoin: 840k blocks in ~3.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // LyraCoin: 3 days
static const int64 nTargetSpacing =  6 * 60; // LyraCoin: 6 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LyraCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LyraCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4d32b571d7b6ee12fe947cffa21544a068336f89d21ad6f14dfc0ea2720181b9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC 06/Mar/2018 BBC, Critically ill man is former Russian spy";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f25423151ed54137ecbf2f0b87798b111539f5c30b08425a725f1e7f650e8ff3cdbce70761bdccc89366eac5af59d450831a60e73619cbd74320d7f1f95d286e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515207931;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085839144;

        if (fTestNet)
        {
            block.nTime    = 1515207937;
            block.nNonce   = 385871415;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x6758431344b32f74abbacbbc88717030072a839f0d32ffa5d110a822a3e20cac"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LyraCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LyraCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LyraCoinMiner(CWallet *pwallet)
{
    printf("LyraCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lyracoin-miner");
    ...
    try { loop {
        ...
        printf("Running LyraCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LyraCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LyraCoinMiner, pwallet));
}
